...
{
    return header->e_ident[EI_CLASS] ==ELFCLASSNONE ||header->e_ident[EI_CLASS] ==ELFCLASS32;//as we are making 32 bit os.
}
...
{
    return header->e_type == ET_EXEC && header->e_entry >= TMOS_PROGRAM_VIRTUAL_ADDRESS;
}

static bool elf_has_program_header(struct elf_header* header)
{
    //if program header is there or not.
...
{
    //header the memory first address.
    return file->elf_memory;
}
...
{
    //The "string table" in an ELF file stores: Symbol Names,Section Names,Error Messages and Descriptive Text,Dynamic Linking Information
...
//note:in this elf file , we are not considering the shared or relocatable file.
